...
object IndexHelper {
  ...
  def createIndex[T](list: List[(T, String)], dir: Directory) = {
    def indexRecipes(writer: IndexWriter, f: (T, String)) = {
  ...
  def searchText(text: String, reader: Directory): List[(String, String, Float)] = {
    val searcher = new IndexSearcher(DirectoryReader.open(reader));

    for (i <- getScoreDocs(text, reader).toList.map(f => (searcher.doc(f.doc).get("documentID"), searcher.doc(f.doc).get("content"), f.score))) yield i

  }

  val threshold = 0.97f
    val NonSpaceSymbols = """[\~\`\!\@\#\$\%\^\&\*\(\)\-\_\+\=\{\}\[\]\|\\\/\:\;\"\'\<\>\,\.\?]"""
  def getCurrentCluster[T](searchResult: List[(T, String, Float)]) = {
    require(!searchResult.isEmpty, "Search result should not empty, atleast the document itself should be returned")
    val init: (List[T], Float) = (List(),  searchResult.head._3)
